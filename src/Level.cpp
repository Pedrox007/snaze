...
bool Level::pass_fruit()
{
    ...
    if (current_fruit >= (int)fruits.size())
        return false; //Retorna falso caso o index ultrapasse o número de frutas do mapa
...
pair<int, int> Level::get_snake_initial_pos()
{
    ...
    for (int i = 0; i < dimensions.first; i++)
    {
        for (int j = 0; j < dimensions.second; j++)
        {
            ...
            if (get_map_object(pos) == SNAKE_INITIAL_POSITION)
                return pos;
        }
    }
